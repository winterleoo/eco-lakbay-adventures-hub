...
serve(async (req)=>{
  ...
  try {
    ...
    let subject = '';
    let htmlBody = '';
    let shouldSend = true; // --- NEW ---: Flag to control email sending
    ...
    switch(status){
      ...
      case 'rejected':
        ...
        break;
      case 'archived':
        subject = `Your destination "${destinationName}" has been archived on EcoLakbay`;
        htmlBody = `<p>Hi ${ownerName},</p><p>This is a notification that your destination, <strong>${destinationName}</strong>, has been archived and is no longer visible on the EcoLakbay platform.</p><p>If you believe this is in error or wish to reactivate your listing in the future, please contact our support team.</p><p>Best regards,<br>The EcoLakbay Team</p>`;
        break;
      default:
        // For any other status (like 'pending'), we won't send an email.
        shouldSend = false;
        break;
    }
    if (!shouldSend) {
    ...
    const resendResponse = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        from: 'EcoLakbay <no-reply@eco-lakbay.com>',
        to: [
          ownerEmail
        ],
        subject: subject,
